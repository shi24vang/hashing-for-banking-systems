...
std::vector<int> CubicProbing::sort(std::vector<int>& v){
	int s=0;
	int e=v.size()-1;
	mergesort(v,s,e);
	return v;
}
void CubicProbing::merge2sort(std::vector<int> &a,int s,int e){
	int mid=s+(e-s)/2;
	int l1=mid-s+1;
	int l2=e-mid;
	std::vector<int> f1;
...
void CubicProbing::mergesort(std::vector<int> &a,int s,int e){
	if(s>=e){
		return;
	}
	int mid=s+(e-s)/2;
